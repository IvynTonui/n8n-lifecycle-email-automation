...
## What I learned:
How to make an LLM safe enough to run unreviewed in production (system-prompt guardrails, structured output, token→URL substitution, factual grounding); how to design idempotent scheduled pipelines where re-runs don't double-act; the subtle time-window bugs that live in cooldown/throttle logic; and to trust the live schema over the docs every time.
